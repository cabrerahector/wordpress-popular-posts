...
xhr.onreadystatechange=function(){if(4===xhr.readyState&&200<=xhr.status&&300>xhr.status){if('function'===typeof callback){callback.call(undefined,xhr.response)}}};xhr.send(('POST'==method?args:null))};const theme=function(wpp_list){let base_styles=document.createElement('style'),dummy_list=document.createElement('ul');dummy_list.innerHTML='<li><a href="#"></a></li>';wpp_list.parentNode.appendChild(dummy_list);let dummy_list_item_styles=getComputedStyle(dummy_list.querySelector('li')),dummy_link_item_styles=getComputedStyle(dummy_list.querySelector('li a'));base_styles.innerHTML='.wpp-list li {font-size: '+dummy_list_item_styles.fontSize+'}';base_styles.innerHTML+='.wpp-list li a {color: '+dummy_link_item_styles.color+'}';wpp_list.parentNode.removeChild(dummy_list);let wpp_list_sr=wpp_list.attachShadow({mode:"open"});wpp_list_sr.append(base_styles);while(wpp_list.firstElementChild){wpp_list_sr.append(wpp_list.firstElementChild)}};return{get:get,post:post,ajax:ajax,theme:theme}})();const fetchWPPWidget=(widget_placeholder)=>{let widget_id_attr=widget_placeholder.getAttribute('data-widget-id'),method='GET',url='',headers={'X-WP-Nonce':wpp_params.token},params='';if(widget_id_attr){url=wpp_params.apiUrl+'/v1/popular-posts/widget/'+widget_id_attr.split('-')[1];params='is_single='+wpp_params.postId+(wpp_params.lang?'&lang='+wpp_params.lang:'')}else{method='POST';url=wpp_params.apiUrl+'/v2/widget?is_single='+wpp_params.postId+(wpp_params.lang?'&lang='+wpp_params.lang:'');headers['Content-Type']='application/json';let json_tag=widget_placeholder.parentNode.querySelector('script[type="application/json"]');if(json_tag){let args=JSON.parse(json_tag.textContent.replace(/[\n\r]/g,''));params=JSON.stringify(args)}}
WordPressPopularPosts.ajax(method,url,params,function(response){renderWPPWidget(response,widget_placeholder)},headers)}
const renderWPPWidget=(response,widget_placeholder)=>{widget_placeholder.insertAdjacentHTML('afterend',JSON.parse(response).widget);let parent=widget_placeholder.parentNode,sr=parent.querySelector('.popular-posts-sr'),json_tag=parent.querySelector('script[type="application/json"]');if(json_tag)
...
let event=new Event("wpp-onload",{"bubbles":!0,"cancelable":!1});parent.dispatchEvent(event)};(function(){if(!Object.keys(wpp_params).length){console.error('WPP params not found, if you are using a JS minifier tool please add wpp.min.js to its exclusion list');return}
...
if(do_request){WordPressPopularPosts.post(wpp_params.apiUrl+'/v2/views/'+post_id,"_wpnonce="+wpp_params.token+"&sampling="+wpp_params.sampling+"&sampling_rate="+wpp_params.samplingRate,function(response){wpp_params.debug&&window.console&&window.console.log&&window.console.log(JSON.parse(response))})}}})();const handleWPPWidgets=()=>{if(!Object.keys(wpp_params).length){return}
const widget_placeholders=document.querySelectorAll('.wpp-widget-placeholder, .wpp-widget-block-placeholder, .wpp-shortcode-placeholder');let w=0;while(w<widget_placeholders.length){fetchWPPWidget(widget_placeholders[w]);w++}
const sr=document.querySelectorAll('.popular-posts-sr');if(sr.length){for(let s=0;s<sr.length;s++){WordPressPopularPosts.theme(sr[s])}}};if('loading'!==document.readyState){handleWPPWidgets()}else{document.addEventListener('DOMContentLoaded',function(){handleWPPWidgets()})}